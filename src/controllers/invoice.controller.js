...
const createInvoice = asyncHandler(async (req, res) => {
    ...
    const user = await User.findOne({ _id: lawyerId, firmId });
    if (!user) {
    ...
    if (!clientId && caseId) {
        ...
        // SECURITY: IDOR Protection - Query with firmId check
        const contract = await Order.findOne({ _id: contractId, firmId });
...
const getInvoices = asyncHandler(async (req, res) => {
    ...
    const firmId = req.firmId; // From firmFilter middleware
    const user = await User.findOne({ _id: req.userID, firmId });
    if (!user) {
        throw CustomException('User not found!', 404);
    }
...
const deleteInvoice = asyncHandler(async (req, res) => {
    ...
    if (invoice.applyFromRetainer > 0 && invoice.retainerTransactionId) {
        ...
        if (firmId) {
            ...
            retainerQuery.lawyerId = req.userID;
        }
        const retainer = await Retainer.findOne(retainerQuery);
        if (retainer) {
...
const voidInvoice = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (invoice.applyFromRetainer > 0 && invoice.retainerTransactionId) {
            ...
            if (retainer) {
                await retainer.deposit(invoice.applyFromRetainer, { session });
            }
        }
...
const applyRetainer = asyncHandler(async (req, res) => {
    ...
    try {
        await invoice.applyRetainer(amount, retainerId, req.userID, { session });
...
const generateXML = asyncHandler(async (req, res) => {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${sanitizeFilename(`invoice-${invoice.invoiceNumber}.xml`)}"`);
    res.send(xml);
});
